...
pub struct ImageHandler {
    ...
    segments: lru_cache::LruCache<u16, Vec<LRIT>>
    //files: Vec<_>
        ...
        if !segmented {
            ...
            if let Some(noaa) = &lrit.headers.noaa {
                if noaa.noaa_compression == 5 {
                    ...
                    if let Ok(mut file) = std::fs::File::create(self.output_root.join(&annotation.text).with_extension("gif")) {
            ...
            // save raw pixel data
            let img: image::GrayImage = image::GrayImage::from_raw(ihs.num_columns as u32, ihs.num_lines as u32, data).unwrap_or_else(|| {
...
impl ImageHandler {
    fn write_image_from_segments(&self, mut segments: Vec<LRIT>) {
        ...
        let ihs = segments.first().unwrap().headers.img_strucutre.as_ref().expect("img_structure header").clone();
        assert_eq!(ihs.bits_per_pixel, 8, "Found non grayscale image: {:?}", ihs);
        let seg = segments.first().unwrap().headers.img_segment.as_ref().expect("img_segment header").clone();
        let ann = segments.first().unwrap().headers.annotation.as_ref().expect("annotation header").clone();

        let num_segments = segments.len();

        //assert_eq!(ihs.num_lines * seg.max_segment, seg.max_column, "segment max_col doesn't match num_lines*max_segment");
        assert!(self.segments.len() <= seg.max_segment as usize, "too many segments: {} <= {}", self.segments.len(), seg.max_segment);
        ...
        match image::GrayImage::from_raw(ihs.num_columns as u32, seg.max_row as u32, pixels) {
            Some(img) => {
                ...
                info!("segmented ({} of {}), {}", num_segments, seg.max_segment, out_name.display());
                img.save(out_name);
            },
            None => {
                /*
...
impl TextHandler {
    pub fn new() -> TextHandler {
        TextHandler {
            output_root: PathBuf::from("/tank/achin/tmp/goes_out")
        }
    }
}
